...
FFT_SP::FFT_SP(unsigned int N, std::function<void(DspTask &task)> callback,
               std::shared_ptr<cl::Context> clContext, std::shared_ptr<cl::Program> clProgram)
        : DspTask(DspTask::_createId(), DspTask::FFT, callback, clContext, clProgram),
          _buffer_size(sizeof(float) * (2 * N + PAD + PAD)), _N(N)
{
    ...
    FFT_SP::generateTwiddleFactors(N, _bufferTwiddle);
    ...
    _clKernel = std::make_shared<cl::Kernel>(*_clProgram, "ocl_DSPF_sp_fftSPxSP");
...
std::vector<cl::Event> FFT_SP::_assignClInputBuffersToQueue(std::shared_ptr<cl::CommandQueue> clCmdQueue){
    try{
...
std::vector<cl::Event> FFT_SP::_assignClOutputBuffersToQueue(std::shared_ptr<cl::CommandQueue> clCmdQueue,
...
unsigned int FFT_SP::getN(){
    return _N;
}

std::size_t FFT_SP::getBufferSize(){
    return _buffer_size;
}

float* FFT_SP::getInputBuffer(unsigned int index){
    if (index == 0)
        return _bufferInput;
    else {
        std::cout << "WARNING: requested non available input buffer in FFT. returning nullptr" << std::endl;
        return nullptr;
    }
}

float* FFT_SP::getOutputBuffer(unsigned int index){
    if (index == 0)
        return _bufferOutput;
    else {
        std::cout << "WARNING: requested non available output buffer in FFT. returning nullptr" << std::endl;
        return nullptr;
    }
}

void FFT_SP::setInputBuffer(std::vector<std::complex<float>>& values){
    if (values.size() != _N){
...
std::shared_ptr<std::vector<std::complex<float>>> FFT_SP::getInputBufferCopy(){
...
std::shared_ptr<std::vector<std::complex<float>>> FFT_SP::getOutputBufferCopy(){
...
void FFT_SP::generateTwiddleFactors(unsigned int N, float *buffer){
    unsigned int i, j, k;

    for (j = 1, k = 0; j <= N >> 2; j = j << 2) {
        for (i = 0; i < N >> 2; i += j) {
            buffer[k]     = (float) sin (2.0 * M_PI * (float)i / (float)N);
            buffer[k + 1] = (float) cos (2.0 * M_PI * (float)i / (float)N);
            buffer[k + 2] = (float) sin (4.0 * M_PI * (float)i / (float)N);
            buffer[k + 3] = (float) cos (4.0 * M_PI * (float)i / (float)N);
            buffer[k + 4] = (float) sin (6.0 * M_PI * (float)i / (float)N);
